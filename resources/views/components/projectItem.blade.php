<div class="w-full max-w-[80%] mx-auto flex flex-col justify-center items-center gap-6">
    ...
    {{-- {!Array.isArray(projects) ? <div class="text-center pt-6 text-amber-300 font-semibold"><p>No projects available!</p></div>:  <div class="w-full  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 place-items-center px-4 rounded-lg">
        ...
        <div class="border-2 w-full h-[400px] p-3 rounded-lg hover:-translate-y-2 transition duration-500 ease-in-out hover:cursor-pointer " key={project.name}>
          ...
          <div class="w-full h-[20%] pt-3">
            <div class="w-full flex justify-between items-center">
                <h1 class="text-xl text-blue-700 font-semibold"><a class="hover:underline" href={project.url} target="_blank" style={!project.active ? { pointerEvents: "none", opacity: 0.5 } : {}}>{project.name}</a></h1>
                <p class="text-xs text-blue-700 font-bold">{project.tech}</p>
            </div>
            <p class={`ext-sm font-light text-slate-200 pt-3`}> {project.description} </p>
